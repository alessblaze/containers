...
# Check if the error log exists and print its contents if there are any errors
if [[ -f "/tmp/libfs_error.log" && -s "/tmp/libfs_error.log" ]]; then
    echo "Error log found at /tmp/libfs_error.log. Here are the details:"
    cat /tmp/libfs_error.log
else
