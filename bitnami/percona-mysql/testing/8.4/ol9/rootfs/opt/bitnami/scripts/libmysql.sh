...
mysql_validate() {
    ...
    if is_boolean_yes "$DB_ENABLE_BACKUP"; then
        if [[ -z "$DB_BACKUP_USER" ]]; then
            print_validation_error "The environment variable $(get_env_var BACKUP_USER) is empty or not set, but $(get_env_var BACKUP_ENABLE) is set to 'yes'. Please provide a backup user."
        fi
        if [[ -z "$DB_BACKUP_PASSWORD" ]]; then
            print_validation_error "The environment variable $(get_env_var BACKUP_PASSWORD) is empty or not set, $(get_env_var BACKUP_ENABLE) is set to 'yes'. Please provide a password for the backup user."
        fi
        if [[ "${DB_BACKUP_PASSWORD:-}" = *\\* ]]; then
            backslash_password_error "$(get_env_var BACKUP_PASSWORD)"
        fi
    fi
...
########################
# MySQL Backup configure
#
#
########################

mysql_initialize_backup() {
    info "Configuring backups"
    mysql_grant_backup_user_permissions "$DB_BACKUP_USER"

}
mysql_grant_backup_user_permissions() {
    local -r user="${1:?user is required}"

    mysql_execute "mysql" "$DB_ROOT_USER" "$DB_ROOT_PASSWORD" <<EOF
GRANT BACKUP_ADMIN, PROCESS, RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO '$user'@'%';
GRANT SELECT ON performance_schema.log_status TO '$user'@'%';
GRANT SELECT ON performance_schema.keyring_component_status TO '$user'@'%';
GRANT SELECT ON performance_schema.replication_group_members TO '$user'@'%';
flush privileges;
EOF
}
...
EOF
        ...
        if [[ -n "$DB_BACKUP_USER" ]]; then
           local -a args=("$DB_BACKUP_USER")
           [[ -n "$DB_BACKUP_PASSWORD" ]] && args+=("-p" "$DB_BACKUP_PASSWORD")
           [[ -n "$DB_AUTHENTICATION_PLUGIN" ]] && args+=("--auth-plugin" "$DB_AUTHENTICATION_PLUGIN")
           mysql_ensure_optional_user_exists "${args[@]}"
        fi
        [[ -n "$DB_REPLICATION_MODE" ]] && mysql_configure_replication
        [[ -n "$DB_ENABLE_BACKUP" ]] && mysql_initialize_backup
