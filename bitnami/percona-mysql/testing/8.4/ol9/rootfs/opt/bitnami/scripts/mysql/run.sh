...
#Start Supercronic for backups
if [ "${DB_ENABLE_BACKUP:-}" == "yes" ]; then
    info "** Starting Backup Cron Job **"

    # Start the cron job using supervisorctl
    supervisorctl start supercronic
else
    info "Backup Disabled Backup cron job not started."
fi
